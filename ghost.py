...
# Tool made by Crank#0597
# Edit to your liking. Will always be open source.
...
def menu():
    ...
    print(logo)
    print(f"""
    {W}\033[2;30m Choose an option.{w}
   {d}<-=-=-=-=-=-=-=-=-=->
    {w}{y}  01{p} User Recon :    {d} Username reconnaissance.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  02{p} Facebook Dumper :   {d} Dump Facebook information.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  03{p} Mail Finder :   {d} Find somone's E-Mail via name.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  04{p} Google Dorker :     {d} Dork with google.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  05{p} Phone Info :    {d} Get info about a phone number.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  06{p} DNS Lookup :    {d} Domain name system lookup
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  07{p} Who Is Lookup :  {d} WhoIs Lookup.
     <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  08{p} Sub Lookup :   {d} Subdomain lookup.
     <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  09{p} Host Finder :   {d} Find domain host.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  10{p} DNS Finder :   {d} Find host domain name system.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  11{p} Reverse IP Lookup :    {d} Reverse IP lookup.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  12{p} IP Location :   {d} IP geolocation.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  13{p} Bit.ly Bypass : {d} Bypass all bit.ly urls.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  14{p} Github Lookup : {d} Dump GitHub information.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}{y}  15{p} TempMail : {d}      Generate a temporary email and inbox.
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
    {w}  {R}00 Exit :         {d} Exit the program {w}{d}
    <-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=->
        """)
    mainmenu()
...
def mainmenu():
    while True: 
        try:
            cmd = input(f"{space}{w}{lr}>{w} Choice:{y} ")
            if int(len(cmd)) < 6:
...
def iplocation():
    ...
    print(w+lines)
    getpass(space+"Press enter to return to the previous menu ")
    menu()
...
class Facebook():
    ...
    def user_token(self):
        ...
        if find == None:
            exit(r+"ERROR: Failed to get token"+w)
        else:
            return find.group(1)
...
def settings():
    ...
    \__> |  | \__/ .__/  | {lr}
    =======================================
    {y}Welcome to the main menu! {w}{lr}
    {G}{r}This tool is made for educational purposes only.{w}{lr}
    {y}Creeper was made by Crank#0597{lr}
    =======================================
""")
    print(f"""\
...
def temp_mail_gen(): 
    ...
    def deleteMail():
        ...
        print(f'\n' + f"{r}Disposing your email address{d}: " + mail + '\n')
...
if __name__ == "__main__":
    ...
    if len(arg) == 1: menu()
        ...
        if arg[1] == "update":
            if which("termux-setup-storage"): path = "$PREFIX/bin/Ghost"
            else:
                if os.path.isdir("/usr/local/bin/"): path = "/usr/local/bin/Ghost"
                else: path = "/usr/bin/sigit"
            os.system(f"wget https://raw.githubusercontent.com/cranksh/Ghost-Toolkit/main/ghost.py -O {path} && chmod +x {path}")
            print(f"{b}>{w} Ghost has been updated!")
        ...
        else: exit(r+"* Command not found: "+str(arg[1:]).replace("[","").replace("]",""))
    else: exit(r+"* Command not found: "+str(arg[1:]).replace("[","").replace("]",""))                   
